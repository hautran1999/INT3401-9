...
def enhancedFeatureExtractorDigit(datum):
    ...
    lines = 0
    horizontalLine = [features[(DIGIT_DATUM_WIDTH / 2, i)]
    ...
    lines = 0
    verticalLine = [features[(i, DIGIT_DATUM_HEIGHT / 2)]
...
def enhancedPacmanFeatures(state, action):
    ...
    closestFood = min(
        map(lambda x: util.manhattanDistance(pacmanPosition, x), food))
    features['foodNum'] = len(food)
    features['closestFood'] = 1.0 / closestFood

    ghosts = state.getGhostPositions() or [(inf, inf)]
    closestGhost = min(
        map(lambda x: util.manhattanDistance(pacmanPosition, x), ghosts))
    ...
    closestCapsule = min(
        map(lambda x: util.manhattanDistance(pacmanPosition, x), capsules))
