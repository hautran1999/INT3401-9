    ...
    def trainAndTune(self, trainingData, trainingLabels, validationData, validationLabels, Cgrid):
        ...
        for c in Cgrid:
            ...
            for iteration in range(self.max_iterations):
                ...
                for datum, label in zip(trainingData, trainingLabels):
                    ...
                    if predictedLabel == label:
                        continue
                    predictedLabelWeights = self.weights[predictedLabel]
                    labelWeights = self.weights[label]
                    tau = ((predictedLabelWeights - labelWeights) * datum + 1.0) / (2 * (datum * datum))
                    tau = min(c, tau)
                    self.weights[predictedLabel] -= datum * tau
                    self.weights[label] += datum * tau
            ...
            for datum, label in zip(validationData, validationLabels):
                predictedLabel = self.classify(datum)[0]
                mistakes += int(predictedLabel != label)
